...
- extend `tools/scripts/coordination.py` instead of creating a second coordination runtime
...
- add local scheduled execution guidance or a lightweight runner so `tools/scripts/coordination.py remind` can run every minute and escalate overdue scopes without pretending CI owns local coordination state
...
- `tools/scripts/coordination.py` supports plural active workstreams with per-scope freshness checks and tests before any reporter path becomes mandatory
...
- split coordination reporting across multiple runtimes instead of extending `tools/scripts/coordination.py`
